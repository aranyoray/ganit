...
struct PlaygroundARContainer: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> ARView {
        ...
        // Tap gesture: spawn blocks on empty space, count on existing blocks
        let tapGesture = UITapGestureRecognizer(
            target: context.coordinator,
            action: #selector(PlaygroundCoordinator.handleTap(_:))
        )
        arView.addGestureRecognizer(tapGesture)

        // Pan gesture: drag / stack blocks
        let panGesture = UIPanGestureRecognizer(
            target: context.coordinator,
            action: #selector(PlaygroundCoordinator.handlePan(_:))
        )
...
class PlaygroundCoordinator: NSObject, ARSessionDelegate {
            ...
            if let planeAnchor = anchor as? ARPlaneAnchor,
               planeAnchor.alignment == .horizontal {
                ...
                DispatchQueue.main.async { [weak self] in
                    self?.onReady()
                    self?.onStatusUpdate("Surface detected! Tap to add blocks.")
                }
                setupPlayground()
    ...
    private func spawnBlock(at position: SIMD3<Float>) {
        ...
        // Start small, animate to full size
        ...
        // Update all block colors based on new count
    ...
    @objc func handleTap(_ recognizer: UITapGestureRecognizer) {
        ...
        if let entity = arView.entity(at: location),
           entity.name.hasPrefix("block_") {
            ...
            transform.scale = originalScale * 1.4
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.12) {
                ...
                entity.move(to: reset, relativeTo: entity.parent, duration: 0.1)
            }
        ...
        if let firstResult = results.first {
            ...
            // Convert to anchor-local coordinates
    ...
    @objc func handlePan(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            // Check for stacking: if dragged block is near another block
            checkStacking(for: entity)

        case .ended, .cancelled:
            if let entity = draggedEntity {
                // Settle block down to surface
                var transform = entity.transform
    ...
    /// When a dragged block is directly above another, stack it.
    private func checkStacking(for entity: ModelEntity) {
        for other in blockEntities {
            ...
            if horizontalDistance < blockSize * 0.6 {
                // Snap on top of the other block
