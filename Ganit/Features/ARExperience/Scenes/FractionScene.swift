...
struct FractionARContainer: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> ARView {
        ...
        // Tap gesture: select / recombine slices
        let tapGesture = UITapGestureRecognizer(
            target: context.coordinator,
            action: #selector(FractionCoordinator.handleTap(_:))
        )
        arView.addGestureRecognizer(tapGesture)

        // Pan gesture: slicing motion
        let panGesture = UIPanGestureRecognizer(
            target: context.coordinator,
            action: #selector(FractionCoordinator.handleSwipe(_:))
        )
...
class FractionCoordinator: NSObject, ARSessionDelegate {
    ...
    private func performSlice() {
        ...
        for i in 0..<targetDenominator {
            // Each slice is a box approximating a pie wedge
            ...
            // Position in a circle, initially at center
        ...
        // Animate slices separating outward
