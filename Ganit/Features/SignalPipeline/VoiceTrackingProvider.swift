...
// MARK: - Voice Tracking Provider (V3 Stub)

/// Tracks speech patterns: rate, hesitation, confidence indicators.
/// V3 stub implementation — full speech analysis in future versions.
#if os(iOS)
@MainActor
class VoiceTrackingProvider: ObservableObject, SignalProvider {
        ...
        AVAudioSession.sharedInstance().requestRecordPermission { [weak self] micGranted in
            Task { @MainActor in
                ...
                guard micGranted else {
                    self.permissionError = "Microphone access is needed for voice analysis. You can enable it in Settings."
                    return
                }
                ...
                SFSpeechRecognizer.requestAuthorization { [weak self] status in
                    Task { @MainActor in
                        ...
                        case .denied, .restricted:
                            self.permissionError = "Speech recognition access is needed for voice analysis. You can enable it in Settings."
                        case .notDetermined:
                            break
                        @unknown default:
                            break
    ...
    private func startRecognition() {
        ...
        do {
            ...
            print("[VoiceTracking] Audio engine failed to start: \(error)")
        }
    }
    ...
    private func processTranscription(_ result: SFSpeechRecognitionResult) {
        ...
        let words = transcript.segments.map(\.substring.lowercased())
        ...
        if wordTimestamps.count > 1 {
            let duration = now.timeIntervalSince(wordTimestamps.first!)
