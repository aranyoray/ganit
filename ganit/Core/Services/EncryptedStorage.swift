...
final class EncryptedStorage: StorageProvider {
    ...
    func saveCodable<T: Codable>(_ key: String, value: T, user: String) -> Bool {
        ...
        do {
            ...
            SignalLogger.signalProviderStarted("EncryptedStorage", available: false) // Codable encode failed for \(key): \(error)")
            return false
        }
    }
    ...
    private func encrypt(data: Data, forKey compositeKey: String) -> Bool {
        do {
            ...
            SignalLogger.signalProviderStarted("EncryptedStorage", available: false) // Encryption failed for \(compositeKey): \(error)")
            return false
        }
    }
